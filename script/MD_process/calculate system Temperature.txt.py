...
def plot_temperature_data(temp_path, total_time, output_dir, max_time, label, color='black', linewidth=2, ax=None, save_plot=False):
    try:
        ...
        ax.set_ylabel('Temperature (K)', fontsize=18, labelpad=20)
        ...
        if save_plot:
            plt.tight_layout()
            output_file = os.path.join(output_dir, f'{label}_temperature_vs_time.png')
...
temp_file_paths = glob.glob('**/*temp.dat', recursive=True)
...
ax_combined.set_ylabel('Temperature (K)', fontsize=20, labelpad=20)
ax_combined.set_title('Combined Temperature vs Time for All Simulations', fontsize=22, fontweight='bold', pad=20)
...
combined_output_file = os.path.join(output_dir, 'combined_temperature_vs_time_2.svg')
